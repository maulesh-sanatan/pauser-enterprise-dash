...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [selectedCompany, companyId, startDate, endDate, triggerSerch]);
  ...
  useEffect(() => {
    ...
    const ctx = document.getElementById("register");

    if (ctx && counts?.data?.users) {
      const userCreationDates = counts.data.users.map(
        (user) => new Date(user.created_at)
      );

      const countsData = userCreationDates.reduce((acc, date) => {
      ...
      const userCountsByDate = sortedDates.map((date) => countsData[date]);
